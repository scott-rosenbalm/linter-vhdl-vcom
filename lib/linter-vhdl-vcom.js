...
export default {
  ...
  lint (textEditor) {
    ...
    const regex = /\*\*([^:]+):.*[^:]*\(([^)]+)\):(.+)/
    const workDir = atom.config.get('linter-vhdl-vcom.workDir')
    let argsString = ''
    atom.config.get('linter-vhdl-vcom.vcomOptions').forEach(function (arg) {
      argsString = `${argsString} ${arg}`
    })
    argsString = `${argsString} "${file}" -work "${workDir}"`
    atom.config.get('linter-vhdl-vcom.includePathsRelativeToTheProject').forEach(function (incPath) {
      argsString = `${argsString} +incdir+${projPath}/${incPath}`
    })
    atom.config.get('linter-vhdl-vcom.includePathsRelativeToTheSourceFile').forEach(function (incPath) {
    ...
    return new Promise(function (resolve) {
      ...
      var messages = []
      var newMessage
      const ignoredCodes = atom.config.get('linter-vhdl-vcom.ignoredVcomErrorCodes')
      ...
      function ignoreCode (line) {
        const codeMatch = line.match(/\(vcom-(\d+)\)/)
        var code
        if (!codeMatch) {
          return false
        } else {
      ...
      rl.on('line', function (line) {
        ...
        if (!parts || parts.length !== 4) {
          if (line.match(/\*\* Error/)) {
            atom.notifications.addError(`linter-vhdl-vcom: vcom command died with this message: ${line}`, { dismissable: true })
          }
